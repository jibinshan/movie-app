...
const { checkAuth, userRole } = require("../middleware/checkauth")
...
router.get("/genre/:movieid",async(req,res)=>{
try {
...
router.put("/genre/:movieid",async(req,res)=>{
  try {
...
router.delete("/genre/:movieid",async(req,res)=>{
    try {
      const updategenre = await movie.findByIdAndUpdate(req.params.movieid,{
          $pull:{
              genre:req.body.genreid
